...
# -*- coding: utf-8 -*-
"""
HYBRID Strategy - Combina múltiplas estratégias com scoring
"""
...
def run_strategy(df, capital=100.0, **params):
    ...
    data['score'] = 0
    data.loc[data['ema_9'] > data['ema_21'], 'score'] += 1
    data.loc[data['close'] > data['ema_50'], 'score'] += 1
    data.loc[(data['rsi'] > 30) & (data['rsi'] < 60), 'score'] += 1
    data.loc[data['macd'] > data['macd_signal'], 'score'] += 2
    data.loc[data['volume'] > data['vol_ma'] * 0.8, 'score'] += 1
    
    data['signal'] = 0
    data.loc[data['score'] >= 5, 'signal'] = 1
    data.loc[data['score'] <= 2, 'signal'] = -1
    ...
    for i, row in data.iterrows():
        ...
        if position is None:
            if row['signal'] == 1 and row['score'] >= 5:
            ...
            elif row['close'] > position['entry'] + row['atr']:
                new_stop = position['entry'] + row['atr'] * 0.5
                if row['low'] < new_stop:
                    exit_price = new_stop
    ...
    if not trades:
        return {"strategy": "HYBRID", "capital_start": capital, "capital_end": capital, "profit": 0, "win_rate": 0, "max_dd": 0, "n_trades": 0}
